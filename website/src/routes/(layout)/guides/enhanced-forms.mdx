...
However, there are a few small things to keep in mind during development. First, you need to pass a schema to `formAction$` with `zodForm$` so that we can also validate the submitted data on the server. Since the validation in this case is server-side, it cannot be manipulated by the user.
...
You have to supplement the paths to arrays, booleans, files and numbers. Below you will find a code example of our special form from the <A href="/playground/special">playground</A>.
...
Not every website needs progressively enhanced forms. However, if yours does, you can find more code examples <a href={`${import.meta.env.VITE_GITHUB_PLAYGROUNDS_URL}/qwik/src/routes/actions`} target="\_blank" rel="noreferrer">here</a> in our GitHub repository.
