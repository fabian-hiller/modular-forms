...
This API design <Solid>leads to an almost completely</Solid><Qwik>results in a fully</Qwik> type-safe form. For example, if you change the name of a field, TypeScript will immediately alert you. Also, you can't mistype when adding an error message, because it is taken from the provided state of the field. More about this on the next page.
