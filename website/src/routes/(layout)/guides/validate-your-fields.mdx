...
> Note that the validation of Modular Forms, except for <A href="/guides/handle-submission">server actions</A>, takes place in the browser and can be manipulated by the user. Make sure that the values are validated again on the server before you process them or store them in your database.
...
export default function App() {
  const [loginForm, { Form, Field }] = createForm<z.input<typeof schema>>({
