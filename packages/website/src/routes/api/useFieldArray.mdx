...
> The `useFieldArray` primitive is used internally in the <A href="/api/FieldArray">`FieldArray`</A> component. It's better to use the <A href="/api/FieldArray">`FieldArray`</A> component and only if there is a good reason to switch to the `useFieldArray` primitive. For example, to retrieve only the reactive `error` of the field array, you should use the <A href="/api/getError">`getError`</A> method instead.
