...
A date or a number representing a date must be converted to a string before it can be passed to an `<input type="date" />` field, for example. Since it is a calculated value, you can use `useComputed$` for this.
