...
> The `handleSubmit` method is used internally in the [`Form`](form) component. It is better to leave the submission of the form to the [`Form`](form) component and only when there is a good reason to switch to the `handleSubmit` method to build your own solution.
...
Errors thrown by the `submitAction` function are made available via the `response` property of the [`ModularForm`](modularform) object to display them to your user.
