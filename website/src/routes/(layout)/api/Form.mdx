...
If JavaScript is available, we prevent the page from being reloaded and send the data to the server as JSON via JavaScript. To change this, you can set `encType` to send the data as `FormData` and `reloadDocument` to not prevent the page from reloading.
