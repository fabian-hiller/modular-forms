...
For a text input field you simply add the `value` attribute<SolidPreactOrReact>{' '}and pass the value of the field or in case of `undefined` or `null` an empty string</SolidPreactOrReact>:
...
export const NumberInput = forwardRef<HTMLInputElement, NumberInputProps>(
  (props, ref) => {
    ...
    return <input {...props} ref={ref} type="number" value={value.value} />;
  }
);
```
...
export const DateInput = forwardRef<HTMLInputElement, DateInputProps>(
  (props, ref) => {
    ...
    return <input {...props} ref={ref} type="date" value={value.value} />;
  }
);
```
