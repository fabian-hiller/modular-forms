...
```tsx
import { createForm, Form, SubmitHandler } from '@modular-forms/solid';
...
You can access errors thrown by your code in <Solid>`onSubmit`</Solid><Qwik>`onSubmit$`</Qwik> via `loginForm.response` to display them in your UI. For server-side code, we are a bit more cautious, as secret information might be preserved here, and therefore only output a generic error message. Therefore, for a better user experience, you should intentionally return the <A href="/api/FormResponse">`FormResponse`</A> object in form actions in case of an error.
