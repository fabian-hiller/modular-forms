...
By default, the fields are validated according to the rules defined in the `options` of the <Solid><A href="../createForm">`createForm`</A> primitive</Solid><QwikPreactOrReact><A href="../useForm">`useForm`</A> hook</QwikPreactOrReact>. To disable validation you can set `shouldValidate` to `false`.
