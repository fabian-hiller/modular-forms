...
The `initialValue` you define directly at a field has the highest specificity and overrides the initial values you set at <A href="/api/createForm">`createForm`</A> and <A href="/api/reset">`reset`</A> when `useField` is executed. Note that the value is enclosed in an array so that we can also detect `undefined` as a value.
