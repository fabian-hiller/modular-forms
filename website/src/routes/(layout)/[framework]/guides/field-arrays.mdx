...
> Currently there is a bug when rendering lists if the `key` property is specified on the first element. Until this is fixed, you simply need to omit the `key` property. This will cause Preact to completely re-render the list, which will prevent the bug from occurring. If you are interested, you can find the issue <a href="https://github.com/preactjs/signals/issues/361" target="_blank" rel="noreferrer">here</a>.
