...
> The `useField` primitive is used internally in the [`Field`](field) component. It's better to use the [`Field`](field) component and only if there is a good reason to switch to the `useField` primitive.
...
The `initialValue` you define directly at a field has the highest specificity and overrides the initial values you set at [`createForm`](createform) and [`reset`](reset) when `useField` is executed.
